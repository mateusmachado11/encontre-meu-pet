...
const CreateAlertModal = ({ isOpen, onClose, type, onAddPet }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const newPet = {
            ...
            image: imagePreview || `https://placehold.co/400x300/cccccc/FFFFFF?text=${formData.get('pet-name') || 'Novo Pet'}`,
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                    ...
                    <div>
                        ...
                        <div className="bg-gray-100 p-3 rounded-lg">
                            <p id="location-display" className="text-gray-600 mb-2">Nenhuma localização selecionada.</p>
                            <div className="flex gap-2">
                                <button type="button" className="text-sm w-full py-2 px-3 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600">Usar minha localização atual (GPS)</button>
                                <button type="button" className="text-sm w-full py-2 px-3 bg-gray-600 text-white font-semibold rounded-lg hover:bg-gray-700">Selecionar no Mapa</button>
                            </div>
                        </div>
                    </div>
                    {!isAdoption && (
                        <input name="pet-reward" type="text" placeholder="Recompensa (Opcional, ex: R$ 100,00)" className="w-full p-3 border rounded-lg" />
                    )}
                    <div>
                     ...
                     <div>
                        ...
                        <input name="user-phone" type="text" placeholder="Seu Telefone (Opcional)" className="w-full p-3 border rounded-lg" />
                        <p className="text-xs text-gray-500 mt-1">Se não preenchido, o contato será feito pelo chat seguro da plataforma.</p>
                    </div>
